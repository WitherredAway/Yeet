...
class Channel(commands.Cog):
        ...
        async with ctx.channel.typing():
            for channel in ctx.guild.text_channels:
                ...
                async for message in channel.history(limit=1):
                    ...
                    if message.embeds:
                        contents.append(
                            key.search(message.embeds[0].title or "") is not None
                        )
                        contents.append(
                            key.search(message.embeds[0].description or "") is not None
                        )
                if any(contents):
    ...
    async def _state(
        self, ctx: commands.Context, state_key: Literal["locked", "unlocked"] = "locked"
    ):
        channels = []
    ...
    async def set_all_send_message_perms(
        self, ctx: commands.Context, send_messages: bool
    ):
    ...
    @commands.guild_only()
    @_lock.command(name="all", brief="Locks all channels.")
