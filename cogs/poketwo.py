...
class Poketwo(commands.Cog):
    ...
    async def _rarity(self, ctx, rarity):
        ...
        for pkm in self.pk.index:
            if all((self.pk.at[pkm, "catchable"] > 0, self.pk.at[pkm, rarity] > 0)):
                pkm_per_cent = round(
                    self.pk.at[pkm, "abundance"] / self.possible_abundance * 100, 4
                )
        ...
        for pkm in self.pk.index:
            if all(
                (
                    ...
                    self.pk.at[pkm, "slug"].endswith(form),
                )
            ):
                pkm_per_cent = round(
                    self.pk.at[pkm, "abundance"] / self.possible_abundance * 100, 4
                ...
                total_abundance += self.pk.at[pkm, "abundance"]
                pokemon.append(
                    f"> **{self.pk.at[pkm, 'name.en']}** - {pkm_per_cent}% (1/{pkm_out_of})"
                )
    ...
    @chance.command(
        name="region",
        aliases=("r", "gen"),
        help="See the chances of a region and the individual pokémon. Options: Kanto/1, Johto/2, Hoenn/3, Sinnoh/4, Unova/5, Kalos/6, Alola/7, Galar/8.",
    )
    async def _region(self, ctx, region: Union[int, str]):
        ...
        for pkm in self.pk.index:
            if all(
                (self.pk.at[pkm, "catchable"] > 0, self.pk.at[pkm, "region"] == region)
            ):
                total_abundance += self.pk.at[pkm, "abundance"]
